...
class Coins(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.weight = random.choice([1, 2, 3]) #diiferent weight for coins
    ...
    def move(self):
        ...
        if self.rect.top > height:
            self.rect.top = 0
            self.rect.center = (random.randint(40, width - 40), 0)
            self.weight = random.choice([2, 4, 6])
...
while running:
    ...
    for coin in collected:
        score_coin += coin.weight  # increase counter of coins
        new_coin = Coins()
        coins.add(new_coin)
        all_sprites.add(new_coin)

    speed = 1 + (score_coin // 3) 
    #it makes the speed increase every 3 recieved coins, for example:
    # 0-2 coins => speed = 1 
    # 3-5 coins => speed = 2
    # 6-8 coins => speed = 3 ...
